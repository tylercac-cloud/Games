...
Crypta can run on a free model on your own PC through Ollama. Install Ollama from ollama.com/download and start it. In Crypta's settings (⚙) the Local model box then offers one-click downloads: Qwen3 8B (about 5 GB, best, needs about 16 GB of RAM) or Qwen3 4B Instruct (about 2.5 GB, for 8 GB of RAM). When the download finishes she switches to it automatically; Brain = Automatic uses Claude if an Anthropic key is present, otherwise the local model, otherwise the guide. Local answers are free and never leave your PC, but they are slower and less sharp than Claude, especially on long multi-step questions.
